...
void test_affine_transforms(CuTest* tc){
  affine_transform * t =  affine_transfrom_from_parameters(4,4,0,0,1,0,0);
  sp_vector * p = sp_vector_alloc(2);
  sp_vector_set(p,0,0);
  sp_vector_set(p,1,1);
  sp_vector * trans;
  ...
  t =  affine_transfrom_from_parameters(4,4,0,0,1,0,M_PI);
  trans = apply_affine_transform(t,p);
  CuAssertDblEquals(tc,sp_vector_get(trans,0),0,REAL_EPSILON);
  CuAssertDblEquals(tc,sp_vector_get(trans,1),-1,REAL_EPSILON);
  sp_vector_free(trans);
  ...
  t =  affine_transfrom_from_parameters(4,4,0,0,2,0,M_PI);
  trans = apply_affine_transform(t,p);
  ...
  t =  affine_transfrom_from_parameters(4,4,1,0,2,0,M_PI);
  trans = apply_affine_transform(t,p);
  ...
  t =  affine_transfrom_from_parameters(4,4,1,1,2,0,M_PI);
  trans = apply_affine_transform(t,p);
  CuAssertDblEquals(tc,sp_vector_get(trans,0),1,sqrt(REAL_EPSILON));
