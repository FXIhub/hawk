...
real calculate_prob_sparse3(Image *ref, sp_sparse_matrix *img, Image * ref_log, real * fact_log_table)
{
  real sum = 0.0;
  /*  real count = 0;
      count = sp_sparse_matrix_integrate(img); */
...
sp_sparse_matrix ** get_rotated_samples(Image * a, int n_samples,gsl_rng * r,Image * sum,int used){
...
sp_sparse_matrix ** get_rotated_samples2(Image * a, int n_samples,gsl_rng * r,Image * sum,int ** weights, int * _used){
 sp_sparse_matrix ** rotated_samples = malloc(sizeof(sp_sparse_matrix *)*n_samples);
 int perc = n_samples/100;
 int increment = 1;
 int id = 0;
 *_used = n_samples;
 *weights = malloc(sizeof(int)*n_samples);
 for(int i =0;i<n_samples;i++){
   (*weights)[i] = 1;
 }
#ifdef MPI
...
sp_sparse_matrix ** get_rotated_samples3(Image * a, int n_samples,gsl_rng * r,Image * sum, int ** weights, int * _used){
  sp_sparse_matrix ** rotated_samples = malloc(sizeof(sp_sparse_matrix *)*n_samples);
  ...
  *weights = malloc(sizeof(int)*n_samples);
  ...
  rotated_samples = realloc(rotated_samples,sizeof(sp_sparse_matrix *)*(used+1));
  rotated_samples[used] = 0;
...
void tomas_iteration(Image * restore,Image * after,sp_sparse_matrix ** random_orient_samples,int n_samples, int * weights, int used){
  ...
  float ** probs = malloc(sizeof(float *)*used);
  for(int i = id;i<used;i+=increment){
  ...
  for(int j = 0;j<4;j++){
    ...
    for(int i = id;i<used;i+=increment){
      ...
      //      probs[i][j] = calculate_prob_sparse3(rot_restore,random_orient_samples[i],ref_log,fact_log_t);
 ...
 int main(int argc, char ** argv){
  ...
  int * weights;
  int used = 0;
     sp_sparse_matrix ** random_orient_samples = get_rotated_samples3(a,n_samples,r,avg_samples,&weights,&used);
