...
Options * parse_options(int argc, char ** argv){
  ...
  static char help_text[] = 
    ...
    -h: print this text\n\
   \n\
    Note that the operation are applied in the same order\n\
    as they are listed above.\n\
";
  static char optstring[] = "i:o:c:s:vh";
...
int main(int argc, char ** argv){
  ...
  x2 = x1+opts->crop_x-1;
  y2 = y1+opts->crop_y-1;
  Image * cropped = rectangle_crop(input,x1,y1,x2,y2);
  ...
  sp_image_write(scaled,opts->output,0);
  return 0;
}
