...
void find_corners(ImageGeometry * geom, Image ** images, int n_images, int * x0, int *y0, int * x1, int *y1){
  ...
  *y1 = sp_vector_get(Tp,1);
  ...
  for(int i = 0;i<n_images;i++){
    for(int x = 0; x<=sp_image_x(images[i]);x+=sp_image_x(images[i])){
      for(int y = 0; y<=sp_image_y(images[i]);y+=sp_image_y(images[i])){
	sp_vector_set(p,0,x);
	sp_vector_set(p,1,y);
	Tp = apply_affine_transform(geom[i].t,p);
	if(sp_vector_get(Tp,0) < *x0){
	  *x0 = sp_vector_get(Tp,0);
	}
	if(sp_vector_get(Tp,1) < *y0){
	  *y0 = sp_vector_get(Tp,1);
	}
	if(sp_vector_get(Tp,0) > *x1){
	  *x1 = sp_vector_get(Tp,0);
	}
	if(sp_vector_get(Tp,1) > *y1){
	  *y1 = sp_vector_get(Tp,1);
	}
	printf("%g x %g\n",sp_vector_get(Tp,0),sp_vector_get(Tp,1));
	sp_vector_free(Tp);
...
int main(int argc, char ** argv){
  if(argc < 4){
    printf("Usage: %s <geometry file> <imageA> <imageB> ...\n",argv[0]);
    exit(1);
  }
  int n_images;
  ...
  for(int i = 2;i<argc;i++){
    images[i-2] = sp_image_read(argv[i],0);
  }

  Image * stitched = stitch_images(geom,images,n_images);
  sp_image_write(stitched,"stitched.h5",0);
  return 0;
}
